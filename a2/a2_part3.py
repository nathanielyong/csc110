...
def sections_in_semester(schedule: dict[str, tuple[str, str, tuple]], semester: str) \
    ...
    Preconditions:
        ...
        - semester in {'F', 'S'}
    """
    return {schedule[s1] for s1 in schedule if schedule[s1][1] in {semester, 'Y'}}
...
def times_conflict(m1: tuple[str, datetime.time, datetime.time],
    ...
    """
    return (m1[0] == m2[0]) and (m1[1] <= m2[1] < m1[2] or m1[1] < m2[2] <= m1[2])
...
def sections_conflict(s1: tuple[str, str, tuple], s2: tuple[str, str, tuple]) \
    ...
    """
    is_same_term = (s1[1] == s2[1] or s1[1] == 'Y' or s2[1] == 'Y')
    return is_same_term and any({times_conflict(m1, m2) for m1 in s1[2] for m2 in s2[2]})
...
def is_valid(schedule: dict[str, tuple[str, str, tuple]]) -> bool:
    ...
    """
    return all({s1 == s2 or not sections_conflict(schedule[s1], schedule[s2])
                for s1 in schedule for s2 in schedule})
...
def valid_schedules(c1: tuple[str, str, set],
    ...
    Preconditions:
        ...
        - c1 != c2
    """
    return [schedule for schedule in possible_schedules(c1, c2) if is_valid(schedule)]
...
def valid_five_course_schedules(c1: tuple[str, str, set],
    ...
    Preconditions:
        ...
        - c4 != c5
    """
    return [schedule for schedule in possible_five_course_schedules(c1, c2, c3, c4, c5)
...
def is_section_compatible(schedule: dict[str, tuple[str, str, tuple]],
    ...
    Preconditions:
        ...
        - schedule matches the format for a schedule described by the assignment handout.
    """
    return all({not sections_conflict(section, schedule[s1]) for s1 in schedule})
