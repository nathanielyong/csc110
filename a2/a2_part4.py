...
def read_course_data(file: str) -> dict:
    ...
    return {data[course]['courseCode']: transform_course_data(data[course]) for course in data}
...
def transform_course_data(course_data: dict) -> tuple[str, str, set]:
    ...
    """
    sections = {transform_section_data(section) for section in course_data['sections']}
...
def transform_section_data(section_data: dict) -> tuple[str, str, tuple]:
    ...
    Preconditions:
        - section_data is a dictionary containing data about a single section, in the format
          found in course_data_small.json.
    """
    meeting_times = [transform_meeting_time_data(m) for m in section_data['meetingTimes']]
    return (section_data['sectionCode'], section_data['term'], tuple(meeting_times))
...
def transform_meeting_time_data(meeting_time_data: dict) \
    ...
    """
    s = str.split(meeting_time_data['startTime'], ':')
    start_time = datetime.time(int(s[0]), int(s[1]))
    e = str.split(meeting_time_data['endTime'], ':')
    end_time = datetime.time(int(e[0]), int(e[1]))
...
def get_valid_schedules(course_data: dict[str, tuple[str, str, set]],
    ...
    """
    filtered = [filter_by_term(course_data[course_code], term) for course_code in courses]
    c0 = filtered[0]
    c1 = filtered[1]
    c2 = filtered[2]
    c3 = filtered[3]
    c4 = filtered[4]
    return a2_part3.valid_five_course_schedules(c0, c1, c2, c3, c4)
...
def filter_by_term(course: tuple[str, str, set], term: str) -> tuple[str, str, set]:
    ...
    Preconditions:
      - term in {'F', 'S'}
    """
    return (course[0], course[1], {section for section in course[2] if section[1] in {term, 'Y'}})
