...
def coprime_to_2_and_3(n: int) -> list[int]:
    ...
    while nums_so_far[-2] + 6 < n:
        ...
        assert all(math.gcd(2, k) == 1 and math.gcd(3, k) == 1 for k in nums_so_far)
        ...
        assert all(not (math.gcd(2, k) == 1 and math.gcd(3, k) == 1) or k in nums_so_far
                   for k in range(nums_so_far[-1] + 1))
...
def coprime_to_all(primes: set[int], n: int) -> list[int]:
    ...
    while nums_so_far[-x] + m < n:
        assert all(math.gcd(p, k) == 1 for k in nums_so_far for p in primes)
        assert all(nums_so_far[i] + m == nums_so_far[i + x] for i in range(len(nums_so_far) - x))
        assert all(nums_so_far[i] < nums_so_far[i + 1] for i in range(len(nums_so_far) - 1))
        assert all(not all(math.gcd(p, k) == 1 for p in primes) or k in nums_so_far
                   for k in range(nums_so_far[-1] + 1))

        next_number = nums_so_far[-x] + m
        list.append(nums_so_far, next_number)
